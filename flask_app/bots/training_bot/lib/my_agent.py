...
from lib.my_policy_ensemble import AdaptedOnlinePolicyEnsemble
from lib.my_processor import CustomProcessor
...
handler = logging.FileHandler("lib/agentcore.log")
...
class CustomAgent(object):
    ...
    def train_online(self,
        ...
        ensemble = AdaptedOnlinePolicyEnsemble(self.policy_ensemble,
                                        training_trackers,
                                        max_visual_history)

        self.policy_ensemble = ensemble

        ensemble.run_online_training(self.domain, self.interpreter,
                                     input_channel)

    def train_online_preloaded_model(self,
                     training_trackers,  # type: List[DialogueStateTracker]
                     input_channel=None,  # type: Optional[InputChannel]
                     max_visual_history=8,  # type: int
                     **kwargs  # type: **Any
                     ):
        # type: (...) -> None
        from rasa_core.policies.online_trainer import OnlinePolicyEnsemble
        ...
        self.policy_ensemble.training_trackers = training_trackers
