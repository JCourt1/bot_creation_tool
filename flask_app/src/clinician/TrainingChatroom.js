...
export default class Chatroom extends Component<ChatroomProps, {}> {
  ...
  groupMessages(messages: Array<ChatMessage>) {
    ...
    const thintHistoryGroup = [];
    const thintBotActionGroup = [];
    const thintActionListGroup = [];
    const thintKeyOptionsGroup = [];

    for (const message of messages.slice(1)) {
      if (
        // Buttons always have their own group
        lastType === "button" ||
        message.message.type.substring(0, 5) === "thint" ||
        ...
        // INTERCEPT TRAINING HINTS
        if (message.message.type === "thintHistory") {
            thintHistoryGroup.push(message)
        } else if (message.message.type === "thintBotAction") {
            thintBotActionGroup.push(message)
        } else if (message.message.type === "thintActionList") {
            thintActionListGroup.push(message)
        } else if (message.message.type === "thintKeyOptions") {
            thintKeyOptionsGroup.push(message)
        } else {
          // new group
    ...
    this.props.addTrainingHints(thintHistoryGroup, this.props.onButtonClick, "thintHistory");
    this.props.addTrainingHints(thintBotActionGroup, this.props.onButtonClick, "thintBotAction");
    this.props.addTrainingHints(thintActionListGroup, this.props.onButtonClick, "thintActionList");
    this.props.addTrainingHints(thintKeyOptionsGroup, this.props.onButtonClick, "thintKeyOptions");
    return groups;
  }
