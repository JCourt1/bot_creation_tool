...
> {
  ...
  sendMessage = async (messageText: string, payload?: string) => {
    ...
    // await fetch(`${this.props.host}/conversations/${this.props.userId}/say?${getParametersString}`, {credentials: 'same-origin'});
    await axios(`${this.props.host}/conversations/${this.props.userId}/say?${getParametersString}`, {
      method: "get",
  ...
  async fetchMessages() {
    ...
    //   `${this.props.host}/conversations/${this.props.userId}/log?nocache=${Date.now()}`, {credentials: 'same-origin'}
    // );

    const res = await axios(`${this.props.host}/conversations/${this.props.userId}/log?nocache=${Date.now()}`, {
      method: "get",
      withCredentials: true
    });
